...
class TriangleTests {
  ...
  private static void assertShapeVariant(Triangle shape, TriangleVariant expectedType) {
    assertEquals(shape.getVariant(), expectedType, "failed to classify " + shape + " as " + expectedType);
  }
  ...
  private static void assertPerimeter(Triangle shape, int expectedType) {
    assertEquals(shape.calculatePerimeterLength(), expectedType, shape + "is incorrect, should be:" + expectedType);
  }

  @Test
  void testPerimeterAndArea(){
    assertPerimeter(new Triangle(10, 12, -1), 23);
  }
