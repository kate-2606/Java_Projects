...
public class InterpreterTests {
    ...
    public void testNestedSelectCommand() {
        ...
        assertTrue(response.contains("Pete"));
        assertTrue(response.contains("Quentin"));
        response = sendCommandToServer("SELECT * FROM details WHERE (((emai LIKE 'hotmail' AND hairColour=='brown') OR " +
                "(hairColour=='pink' AND email LIKE 'example')) AND (height>=150 AND hairColour!='pink')) OR name=='Quentin';");
        assertFalse(response.contains("Steve"));
        assertFalse(response.contains("Paul"));
